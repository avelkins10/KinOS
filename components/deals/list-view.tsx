...
type SortField = "customerName" | "stage" | "systemSize" | "dealValue" | "createdAt" | "daysInPipeline"
type SortDir = "asc" | "desc"

export function ListView({ deals }: { deals: Deal[] }) {
  ...
  const SortHeader = ({ field, children }: { field: SortField; children: React.ReactNode }) => (
    ...
    >
      {children}
      <ArrowUpDown className={cn("h-3 w-3", sortField === field ? "text-primary" : "text-muted-foreground/30")} />
    </button>
  )
              ...
              >
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-card-foreground">{deal.closer.name}</span>
                  </div>
                </td>
                ...
                <td className="px-4 py-3 text-sm text-muted-foreground">
                  {deal.lender ?? <span className="text-muted-foreground/40">--</span>}
                </td>
                <td className="px-4 py-3 text-right text-sm font-semibold text-card-foreground">
                  {deal.dealValue > 0 ? `$${(deal.dealValue / 1000).toFixed(1)}k` : "--"}
                </td>
                <td className="px-4 py-3 text-right">
                  <span className={cn(
                    "text-sm font-medium",
                    deal.daysInPipeline > 45 ? "text-destructive" : deal.daysInPipeline > 30 ? "text-warning" : "text-muted-foreground"
                  )}>
