...
export function OverviewTab({ deal }: { deal: DealForUI }) {
  const dealActivity = RECENT_ACTIVITY.filter(
    (a) => a.dealId === deal.id,
  ).slice(0, 4);
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-foreground">
                {deal.utilityAccount || "Not provided"}
              </p>
            </div>
          </div>
        </div>
        ...
        {deal.systemSize ? (
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-foreground">
                  {deal.panelCount} x {deal.panelBrand}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              System details will appear after design
            </p>
            <button
              type="button"
              className="mt-3 rounded-lg bg-primary px-3 py-1.5 text-xs font-semibold text-primary-foreground hover:bg-primary/90 transition-colors"
            >
        ...
        {deal.dealValue > 0 ? (
          <div className="space-y-1.5">
            {[
              {
                label: "Base System",
                value: deal.dealValue * 0.97,
                perWatt: deal.systemSize
                  ? (deal.dealValue * 0.97) / (deal.systemSize * 1000)
                  : 0,
              },
              { label: "Equipment Adders", value: 0, perWatt: 0 },
              {
                label: "Adders",
                value: deal.dealValue * 0.03,
                perWatt: deal.systemSize
                  ? (deal.dealValue * 0.03) / (deal.systemSize * 1000)
                  : 0,
              },
            ].map((row, i) => (
              <div
                key={row.label}
                className={`flex items-center justify-between py-2 ${i > 0 ? "border-t border-border/50" : ""}`}
              >
            ...
            {deal.monthlyPayment && (
              <div className="flex items-center justify-between rounded-lg bg-primary/10 px-4 py-3">
                ...
                <span className="text-xl font-bold text-primary">
                  ${deal.monthlyPayment.toFixed(2)}/mo
                </span>
              </div>
            )}
          </div>
