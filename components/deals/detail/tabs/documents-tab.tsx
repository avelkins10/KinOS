...
function getMockDocs(deal: DealForUI): Doc[] {
  ...
  return [
    ...
    {
      id: "doc3",
      name: "Interconnection Authorization",
      recipient: deal.customerName,
      status: "viewed",
      sentAt: "Feb 4, 2026",
      viewedAt: "Feb 4, 2026",
...
export function DocumentsTab({ deal }: { deal: DealForUI }) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1.5">
                {(["sent", "viewed", "signed"] as const).map((s) => {
                  const isReached =
                    ["sent", "viewed", "signed"].indexOf(doc.status) >=
                    ["sent", "viewed", "signed"].indexOf(s);
                  return (
                    <div key={s} className="flex flex-col items-center gap-0.5">
                      <div
                        className={cn(
                          "h-2 w-2 rounded-full",
                          isReached ? statusDots[s] : "bg-border",
                        )}
                      />
                      <span className="text-[8px] text-muted-foreground/50 capitalize">
                        {s}
                      </span>
                    </div>
                  );
