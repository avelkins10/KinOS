...
import { CreditCard, Check, Clock, AlertTriangle, Upload, Send } from "lucide-react"
...
export function FinancingStep({ deal }: { deal: Deal }) {
  if (!deal.lender) {
    return (
      ...
      <div className="flex items-center gap-4 rounded-xl border border-border bg-card p-5">
        ...
        {deal.monthlyPayment && (
          <div className="text-right">
            <p className="text-xl font-bold text-foreground">${deal.monthlyPayment}/mo</p>
            ...
            return (
              <div key={step} className="flex flex-1 flex-col items-center gap-2">
                <div className="flex items-center w-full">
                  {idx > 0 && (
                    <div className={cn("h-0.5 flex-1", idx <= currentFinStep ? "bg-success" : "bg-border")} />
                  )}
                  <div
                    className={cn(
                      "flex h-9 w-9 shrink-0 items-center justify-center rounded-full border-2 transition-all",
                      isDone ? "border-success bg-success" : isCurrent ? "border-primary bg-primary/10" : "border-border bg-muted"
                    )}
                  >
                    {isDone ? (
        ...
        <div className="rounded-xl border border-warning/30 bg-warning/5 p-5">
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={stip.label} className="flex items-center gap-3 rounded-lg border border-border bg-card p-3.5">
                {stip.uploaded ? (
                ...
                <span className={cn("flex-1 text-sm", stip.uploaded ? "text-muted-foreground line-through" : "text-foreground font-medium")}>
                  {stip.label}
                </span>
                {!stip.uploaded && (
                  <button type="button" className="rounded-lg bg-primary/10 px-3 py-1.5 text-xs font-semibold text-primary hover:bg-primary/20 transition-colors">
                    Upload
                  </button>
                )}
              </div>
