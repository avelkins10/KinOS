...
export function ContractingStep({
  ...
  const docs =
    dealDetail?.documentEnvelopes?.length &&
    dealDetail.documentEnvelopes.some((d) => d.id)
      ? dealDetail.documentEnvelopes.map((d) => ({
          id: d.id,
          name: d.title,
          recipient: deal.customerName,
          status: (d.status === "signed"
            ...
            : d.status === "sent" ||
              ...
              ? "sent"
              : "pending") as "pending" | "sent" | "viewed" | "signed",
          sentAt: d.sent_at
            ? new Date(d.sent_at).toLocaleDateString("en-US", {
                month: "short",
                day: "numeric",
                year: "numeric",
              })
