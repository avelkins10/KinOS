...
const urgencyConfig = {
  high: { icon: AlertTriangle, color: "text-destructive bg-destructive/10 border-destructive/20" },
  medium: { icon: Clock, color: "text-warning bg-warning/10 border-warning/20" },
  low: { icon: CheckCircle, color: "text-success bg-success/10 border-success/20" },
}
        ...
        {FINANCING_ALERTS.map((alert) => {
          ...
          return (
            ...
            >
              ...
              <span className="shrink-0 text-xs font-medium opacity-60">{alert.detail}</span>
            </a>
          )
        })}
      </div>
