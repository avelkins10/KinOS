...
export default function ReportsPage() {
  ...
  const avgDealSize = totalRevenue / DEALS.filter((d) => d.dealValue > 0).length

  return (
    <div className="p-6 lg:p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold tracking-tight text-foreground">Reports</h1>
        <p className="mt-1 text-sm text-muted-foreground">Performance metrics and pipeline analytics.</p>
      </div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-foreground">${(avgDealSize / 1000).toFixed(1)}k</p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-foreground">
                {(REPS.filter((r) => r.role === "closer").reduce((s, r) => s + r.closeRate, 0) / REPS.filter((r) => r.role === "closer").length).toFixed(0)}%
              </p>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 rounded-xl border border-border bg-card p-5">
          <h3 className="mb-4 text-sm font-semibold text-foreground">Monthly Deals Closed</h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={monthlyData}>
                ...
                />
                <Bar dataKey="deals" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <h3 className="mb-4 text-sm font-semibold text-foreground">Stage Distribution</h3>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  dataKey="value"
                >
                  {stageDistribution.filter((d) => d.value > 0).map((entry) => (
