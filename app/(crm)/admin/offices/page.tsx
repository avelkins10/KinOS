    ...
    <div className="p-6 lg:p-8">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Manage office locations and team assignments.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {OFFICES.map((office) => {
          ...
          const officeDeals = DEALS.filter((d) =>
            reps.some((r) => r.id === d.closer.id),
          );
          const totalRevenue = officeDeals.reduce(
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {reps.map((rep) => (
                  <div
                    key={rep.id}
                    className="flex items-center gap-2 rounded-full border border-border bg-muted/50 px-2.5 py-1"
                  >
